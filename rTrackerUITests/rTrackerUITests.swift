...
final class rTrackerUITests: XCTestCase {
  ...
  // MARK: - Slider Correction Table

  /// Correction lookup table for setSlider() drag calculations
  /// Maps normalized input position [0.0-1.0] to corrected aim position
  /// Based on empirical data showing iOS slider overshoots at low values
  /// Interpolation is used between entries for smooth correction curve
  private let sliderCorrectionTable: [(input: Float, factor: Float)] = [
    (0.00, 0.00),  // 0% → Reset to 0, no drag needed
    (0.10, 0.06),  // 10% target → aim for 6% to land near 10%
    (0.20, 0.13),  // 20% target → aim for 13%
    (0.30, 0.22),  // 30% target → aim for 22%
    (0.40, 0.32),  // 40% target → aim for 32%
    (0.50, 0.43),  // 50% target → aim for 43%
    (0.60, 0.54),  // 60% target → aim for 54%
    (0.70, 0.65),  // 70% target → aim for 65%
    (0.80, 0.77),  // 80% target → aim for 77%
    (0.90, 0.90),  // 90% target → perfect, no correction needed
    (1.00, 1.01)   // 100% target → slight overshoot attempt (may still land at 99%)
  ]
  ...
  override func setUpWithError() throws {
    ...
    let contactsMonitor = addUIInterruptionMonitor(withDescription: "Contacts Access Authorization")
    { (alert) -> Bool in
    ...
    // Wait for toolbar to initialize before checking for KB button
    sleep(1)
  ...
  func setupContactsAccessAndKateBell() throws {
    ...
    XCTAssertTrue(kbButton.exists, "KB button not found - TESTING flag may not be set or toolbar not initialized")
  ...
  func performReliableDrag(from sourceElement: XCUIElement, to destinationElement: XCUIElement) {
    let sourceCoordinate = sourceElement.coordinate(
      withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
    let destinationCoordinate = destinationElement.coordinate(
      withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
  ...
  /// Assert that a string value (from UI element) is within ±tolerance of expected numeric value
  /// Used for slider-dependent values that may have slight positioning variations
  func assertValueWithinTolerance(
    ...
    guard let actual = Int(actualString) else {
      XCTFail("Could not parse '\(actualString)' as Int. \(message)", file: file, line: line)
      return
    }
    let difference = abs(actual - expected)
    XCTAssertLessThanOrEqual(
      difference, tolerance,
      "Value \(actual) outside tolerance range [\(expected - tolerance)..\(expected + tolerance)]. \(message)",
      file: file, line: line
    )
  }

  /// Assert that a minutes field is at the zero boundary (accepts "59", "00", or "01")
  /// Used for slider-based time fields where minute=0 may be slightly off due to slider positioning
  func assertMinutesAtZero(
    _ actualString: String,
    message: String,
    file: StaticString = #file,
    line: UInt = #line
  ) {
    let validValues = ["59", "00", "01"]
    XCTAssertTrue(
      validValues.contains(actualString),
      "Expected minutes to be 59, 00, or 01, but got '\(actualString)'. \(message)",
      file: file,
      line: line
    )
  }

  /// Assert that a floating point string value is within ±tolerance of expected value
  func assertFloatValueWithinTolerance(
    _ actualString: String,
    expected: Double,
    ...
    guard let actual = Double(actualString) else {
      XCTFail("Could not parse '\(actualString)' as Double. \(message)", file: file, line: line)
      return
    }
    let difference = abs(actual - expected)
    XCTAssertLessThanOrEqual(
  ...
  func performTableCellReorder(
    from sourceCell: XCUIElement, to destinationCell: XCUIElement, insertAbove: Bool = true
  ) {
    DBGLog(
      "Attempting to reorder from '\(sourceCell.staticTexts.firstMatch.label)' to '\(destinationCell.staticTexts.firstMatch.label)'"
    )
    ...
    let reorderButtonWithImage = sourceCell.buttons["Reorder \(cellLabel)"].images[
      "line.horizontal.3"
    ].firstMatch
    if reorderButtonWithImage.exists && reorderButtonWithImage.isHittable {
      ...
      let sourceCoordinate = reorderButtonWithImage.coordinate(
        withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
      let targetCoordinate = destinationCell.coordinate(
        withNormalizedOffset: CGVector(dx: 0.5, dy: destYOffset))
    ...
    if reorderControl.exists && reorderControl.isHittable {
      ...
      let sourceCoordinate = reorderControl.coordinate(
        withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
      let targetCoordinate = destinationCell.coordinate(
        withNormalizedOffset: CGVector(dx: 0.5, dy: destYOffset))
      sourceCoordinate.press(forDuration: 1.6, thenDragTo: targetCoordinate)
    ...
    let reorderButtons = sourceCell.buttons.matching(
      NSPredicate(format: "identifier CONTAINS[c] 'reorder' OR label CONTAINS[c] 'reorder'"))
    if reorderButtons.count > 0 {
      ...
      if reorderButton.exists && reorderButton.isHittable {
        ...
        let sourceCoordinate = reorderButton.coordinate(
          withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
        let targetCoordinate = destinationCell.coordinate(
          withNormalizedOffset: CGVector(dx: 0.5, dy: destYOffset))
        sourceCoordinate.press(forDuration: 1.6, thenDragTo: targetCoordinate)
    ...
    if allButtons.count > 0 {
      ...
      if rightmostButton.exists && rightmostButton.isHittable {
        DBGLog(
          "Strategy 4: Using rightmost button: '\(rightmostButton.identifier)' - '\(rightmostButton.label)'"
        )
        let sourceCoordinate = rightmostButton.coordinate(
          withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
        let targetCoordinate = destinationCell.coordinate(
          withNormalizedOffset: CGVector(dx: 0.5, dy: destYOffset))
        sourceCoordinate.press(forDuration: 1.6, thenDragTo: targetCoordinate)
  ...
  // MARK: - Slider Correction Helper

  /// Interpolate correction factor from lookup table for slider positioning
  /// - Parameter position: Desired normalized position [0.0-1.0]
  /// - Returns: Corrected position to use in drag calculation
  private func interpolateSliderCorrection(for position: Float) -> Float {
    // Handle edge cases
    guard position >= 0.0 && position <= 1.0 else {
      DBGWarn("Position \(position) out of range [0.0, 1.0], clamping")
      return max(0.0, min(1.0, position))
    }

    // Find bracketing entries in correction table
    var lowerEntry = sliderCorrectionTable.first!
    var upperEntry = sliderCorrectionTable.last!

    for i in 0..<sliderCorrectionTable.count - 1 {
      if position >= sliderCorrectionTable[i].input
        && position <= sliderCorrectionTable[i + 1].input
      {
        lowerEntry = sliderCorrectionTable[i]
        upperEntry = sliderCorrectionTable[i + 1]
        break
      }
    }

    // Exact match - no interpolation needed
    if lowerEntry.input == upperEntry.input {
      return lowerEntry.factor
    }

    // Linear interpolation between bracketing entries
    let inputRange = upperEntry.input - lowerEntry.input
    let ratio = (position - lowerEntry.input) / inputRange
    let correctedPosition = lowerEntry.factor + ratio * (upperEntry.factor - lowerEntry.factor)
  ...
  /// Adjust slider to a specific position using simple reset-then-adjust approach
  /// - Parameters:
  ///   - slider: The XCUIElement slider to adjust
  ///   - normalizedPosition: Target position on [0.0, 1.0] range
  /// - Note: Resets slider to 0 first to establish known state, then adjusts to target position
  ///   This approach provides better consistency than direct adjust() calls while being much faster than setSlider()
  func adjustSlider(_ slider: XCUIElement, to normalizedPosition: Float) {
    if normalizedPosition > 0.5 {
        slider.adjust(toNormalizedSliderPosition: 1.0)
    } else {
        slider.adjust(toNormalizedSliderPosition: 0.0)
    }
    usleep(100000)  // Wait 100 msec for slider to settle at 0
    slider.adjust(toNormalizedSliderPosition: CGFloat(normalizedPosition))
  }

  /// Set slider to a specific position using reset-then-drag with absolute screen coordinates
  /// - Parameters:
  ///   - slider: The XCUIElement slider to adjust
  ///   - normalizedPosition: Target position on [0.0, 1.0] range
  /// - Note: Resets slider to 0 first so we know where the handle is, then drags using absolute screen coordinates with empirical correction
  func setSlider(_ slider: XCUIElement, to normalizedPosition: Float) {
    // Special case: For exact 0% or 100%, use XCTest's built-in adjust method
    // The drag-based approach has difficulty reaching exact endpoints
    if normalizedPosition == 0.0 || normalizedPosition == 1.0 {
      slider.adjust(toNormalizedSliderPosition: CGFloat(normalizedPosition))
      usleep(200000)  // Allow slider to settle (0.2 seconds)

      if let finalValue = slider.value as? String {
        DBGLog("Slider set to edge position: \(finalValue) (target: \(normalizedPosition))")
      }
      return
    }

    // Step 1: Apply empirical correction using interpolated lookup table
    // iOS sliders overshoot at low values due to touch handling and handle geometry
    let correctedPosition = interpolateSliderCorrection(for: normalizedPosition)
    let correctionAmount = correctedPosition - normalizedPosition

    DBGLog(
      "Target: \(normalizedPosition), corrected: \(correctedPosition), adjustment: \(correctionAmount)"
    )

    // Step 2: Reset slider to 0 so we know exactly where the handle is
    slider.adjust(toNormalizedSliderPosition: 0.0)
    usleep(200000)  // Allow slider to settle (0.2 seconds)

    // Step 3: Verify reset to 0 - retry once if needed
    var resetSuccess = false
    if let sliderValue = slider.value as? String {
      let numericValue = sliderValue.replacingOccurrences(of: "%", with: "")
      if numericValue == "0" || numericValue == "0.00" {
        resetSuccess = true
      } else {
        // Retry once
        slider.adjust(toNormalizedSliderPosition: 0.0)
        ...
        if let retryValue = slider.value as? String {
          let retryNumeric = retryValue.replacingOccurrences(of: "%", with: "")
          if retryNumeric == "0" || retryNumeric == "0.00" {
            resetSuccess = true
          }
        }
      }
    ...
    // Assert if reset failed - cannot proceed
    XCTAssertTrue(resetSuccess,
      "Cannot reset slider to 0 for coordinate-based drag. Test must fail to prevent swipe gesture.")

    // Step 3.5: Validate start position calculation by attempting small test drag
    // Read initial position (should be 0)
    var initialValue: Int = 0
    if let sliderValue = slider.value as? String {
      initialValue = Int(sliderValue.replacingOccurrences(of: "%", with: "")) ?? 0
    }

    // Get frame and calculate coordinates
    let frame = slider.frame
    let handleRadius: CGFloat = 20  // Estimate - will be validated by test drag
    let startX = frame.origin.x + handleRadius
    let midY = frame.origin.y + (frame.height / 2)

    // Test: Try to drag slider from calculated startX to ~10% position
    let testTargetX = startX + 30  // Small movement ~10%
    let testStartCoord = XCUIApplication().coordinate(withNormalizedOffset: .zero)
      .withOffset(CGVector(dx: startX, dy: midY))
    let testEndCoord = XCUIApplication().coordinate(withNormalizedOffset: .zero)
      .withOffset(CGVector(dx: testTargetX, dy: midY))

    testStartCoord.press(forDuration: 0.05, thenDragTo: testEndCoord)
    usleep(100000)  // Allow drag to complete

    // Verify the test drag changed the slider value
    var testValue: Int = 0
    if let sliderValue = slider.value as? String {
      testValue = Int(sliderValue.replacingOccurrences(of: "%", with: "")) ?? 0
    }

    XCTAssertNotEqual(testValue, initialValue,
      "Test drag did not move slider (was \(initialValue)%, still \(testValue)%). " +
      "Start position calculation is wrong - handleRadius estimate may be incorrect. " +
      "This would cause swipe gesture instead of slider adjustment.")

    // Reset back to 0 for actual target drag
    slider.adjust(toNormalizedSliderPosition: 0.0)
    usleep(200000)

    // Step 4: Calculate target position for actual drag
    DBGLog(
      "Slider frame: origin=(\(frame.origin.x), \(frame.origin.y)), width=\(frame.width), height=\(frame.height)"
    )

    // Step 5: Calculate absolute screen coordinates accounting for handle size
    let usableWidth = frame.width - (handleRadius * 2)  // Range minus space taken by handle
    // Extend drag slightly beyond handleRadius to compensate for systematic undershoot at high values
    let extraExtension: CGFloat = handleRadius + 3  // Add 3 extra pixels to reach 100%
    let targetX = startX + (usableWidth * CGFloat(correctedPosition)) + extraExtension

    DBGLog(
      "Dragging from (\(startX), \(midY)) to (\(targetX), \(midY)) [usableWidth: \(usableWidth), extension: \(extraExtension)]"
    )

    // Step 6: Create absolute coordinates and perform actual target drag
    let startCoord = XCUIApplication().coordinate(withNormalizedOffset: .zero)
      .withOffset(CGVector(dx: startX, dy: midY))
    let endCoord = XCUIApplication().coordinate(withNormalizedOffset: .zero)
      .withOffset(CGVector(dx: targetX, dy: midY))

    startCoord.press(forDuration: 0.05, thenDragTo: endCoord)
    usleep(100000)  // Allow drag to complete (0.1 seconds)

    // Step 6: Verify final position achieved
    if let finalValue = slider.value as? String {
      DBGLog("Slider final position: \(finalValue) (target: \(normalizedPosition))")
    }
  }
  ...
  func testTrackerDemoInstall() throws {
    ...
    if !fnTotalLabel.exists {
      ...
      if totalElements.count > 0 {
        ...
        let possibleElements = app.staticTexts.matching(
          NSPredicate(format: "identifier CONTAINS 'total' OR identifier CONTAINS 'fnVal'"))
        if possibleElements.count > 0 {
          DBGLog(
            "Found possible total element: identifier='\(possibleElements.firstMatch.identifier)', label='\(possibleElements.firstMatch.label)'"
          )
          XCTFail(
            "fnVal_total element not found, but found possible alternative: \(possibleElements.firstMatch.identifier)"
          )
        } else {
  ...
  func testEditTrackerRank() throws {
    ...
    let sourceCoordinate = reorderHandle.coordinate(
      withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
    ...
    guard exerciseCell.exists && weightStatsCell.exists else {
      DBGLog(
        "Required cells not found - Exercise exists: \(exerciseCell.exists), Weight stats exists: \(weightStatsCell.exists)"
      )
    ...
    let reverseReorderHandle = app.buttons["Reorder 🚴 Exercise"].images["line.horizontal.3"]
      .firstMatch
    DBGLog(
      "Reverse reorder handle exists: \(reverseReorderHandle.exists), hittable: \(reverseReorderHandle.isHittable)"
    )

    let reverseSourceCoordinate = reverseReorderHandle.coordinate(
      withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
    let reverseTargetCoordinate = app.tables.cells["configt_☕️🍷 Drinks"].coordinate(
  ...
  func testSearchSetup() throws {
    ...
    for i in 1...5 {
      ...
      XCTAssert(
        trkrSaveButton.waitForExistence(timeout: 5),
        "trkrSave button should appear after closing textbox")
  ...
  func testSearch() throws {
    ...
    XCTAssertEqual(
      odFld.value as! String, "54321",
      "odometer field not last value. Actual value: '\(odFld.value as? String ?? "nil")'")
  ...
  func testSearchClear() throws {
    ...
    let deleteSuccessful = performSwipeToDelete(
      on: carCell, expectedAlertTitle: "Delete tracker 🚗 Car")
  ...
  func testTrackerDemoUse() throws {
    ...
    adjustSlider(slider, to: 0.30)  // value 23
    sleep(1)  // Allow slider to settle
    XCTAssertEqual(sliderEnable.value as! String, "1", "The sliderEnable switch should be On")
    assertFloatValueWithinTolerance(
      fnTotalLabel.label, expected: 35.00, tolerance: 1.0,
      message: "function total is incorrect (slider) before save"
    )

    // test choice buttons enable and non-default values counted
    let choiceSeg = app.segmentedControls[
      "👣rTracker demo_Choices with values (e.g. Likert scale)_choices"]
    ...
    assertFloatValueWithinTolerance(
      fnTotalLabel.label, expected: 38.00, tolerance: 1.0,
      message: "function total is incorrect (choice) before save"
    )
    ...
    let contactMonitor = addUIInterruptionMonitor(withDescription: "Contact Access Prompt") {
      (alert) -> Bool in  // Allow Notifications
    ...
    XCTAssertEqual(
      tbButton.label, expectedTbContent.replacingOccurrences(of: "\n", with: " "),
      "textBox button label not as expected")
    ...
    assertFloatValueWithinTolerance(
      fnTotalLabel.label, expected: 37.00, tolerance: 1.0,
      message: "function total is incorrect after save"
    )
    ...
    XCTAssertEqual(
      tbButton.label, expectedTbContent.replacingOccurrences(of: "\n", with: " "),
      "textBox button label not 'rTracker'")

    // confirm textBox value
    tbButton.tap()
    XCTAssertEqual(
      (tbtv.value as! String).trimmingCharacters(in: .whitespacesAndNewlines), expectedTbContent,
      "The textbox content is incorrect")
  ...
  func testDTslider() throws {
    ...
    // get slider and function value elements
    let slider = app.sliders["👣rTracker demo_Low|High_slider"]
    let fnSliderVal = app.staticTexts["fnVal_slider value"]

    // loop through slider values from 0.0 to 1.0 in 0.1 increments
    for i in 0...10 {  // 0 through 10 inclusive = 11 values (0.0, 0.1, 0.2, ..., 1.0)
      let sliderValue = Float(i) / 10.0
      slider.adjust(toNormalizedSliderPosition: 0.0)
      //usleep(100000)  // wait 100 msec
      slider.adjust(toNormalizedSliderPosition: CGFloat(sliderValue))
      //usleep(500000)  // wait 500 msec
      DBGLog("Slider: \(sliderValue) -> slider.value: \(slider.value ?? "nil") -> fnVal: \(fnSliderVal.label)")
    }
  ...
  func testDTsliderSS() throws {
    ...
    for i in 0...10 {  // 0 through 10 inclusive = 11 values (0.0, 0.1, 0.2, ..., 1.0)
      let sliderValue = Float(i) / 10.0
      setSlider(slider, to: sliderValue)
      DBGLog("Slider: \(sliderValue) -> slider.value: \(slider.value ?? "nil") -> fnVal: \(fnSliderVal.label)")
    }
  ...
  func testModifyNewTracker() throws {
    ...
    // Setup button identifier is addTrkrSetup (not modTrkrConfig)
    ...
    let expectedDefnContent =
      "sum[vnumber] + change_in[vnumber] + vchoice1 - ( vyes/no * vchoice2 ) + vtext + vtextbox + vslider + vchoice2 + vinfo /  2  "
  ...
  func testPopulateNewTracker() throws {
    ...
    for i in 1...8 {
      ...
      adjustSlider(slider, to: Float(i) * 0.1)
      if let sliderValue = slider.value as? String {
  ...
  func testNewTracker() throws {
    ...
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 182.73, tolerance: 1.0, message: "vfunction value record 1")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 160.52, tolerance: 1.0, message: "vfunction value record 2")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 128.42, tolerance: 1.0, message: "vfunction value record 3")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 100.15, tolerance: 1.0, message: "vfunction value record 4")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 69.30, tolerance: 1.0, message: "vfunction value record 5")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 50.42, tolerance: 1.0, message: "vfunction value record 6")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 30.09, tolerance: 1.0, message: "vfunction value record 7")
    app.swipeRight()
    assertFloatValueWithinTolerance(vfuncLabel.label, expected: 13.21, tolerance: 1.0, message: "vfunction value record 8")
    app.buttons["trkrBack"].tap()
  }
  ...
  func testPrivacySetup() throws {
    ...
    adjustSlider(slider, to: 0.15)

    assertValueWithinTolerance(plvlLab.label, expected: 26, tolerance: 1, message: "Privacy level after slider set to 0.15")
    let lvl015 = plvlLab.label
    ...
    adjustSlider(slider, to: 0.60)
    assertValueWithinTolerance(plvlLab.label, expected: 65, tolerance: 1, message: "Privacy level after slider set to 0.60")
    let lvl060 = plvlLab.label
    ctr.tap()
    XCTAssertEqual(ctr.label, "O middle middle")
    sav.tap()

    adjustSlider(slider, to: 0.80)
    assertValueWithinTolerance(plvlLab.label, expected: 84, tolerance: 1, message: "Privacy level after slider set to 0.80")
    let lvl080 = plvlLab.label
    ctr.tap()
    ...
    prv.tap()
    prv.tap()
    XCTAssertEqual(plvlLab.label, lvl015)
    nxt.tap()
    XCTAssertEqual(plvlLab.label, lvl060)
    ...
    XCTAssertEqual(plvlLab.label, lvl015)
    nxt.tap()
    XCTAssertEqual(plvlLab.label, lvl080)
    clr.tap()
  ...
  func tapRightEdge(of textField: XCUIElement) {
    ...
    let rightmostCoordinate = textField.coordinate(
  ...
  func testReminders() throws {
    ...
    */
    ...
    adjustSlider(untilSlider, to: 0.0)  // until before at/from
    XCTAssertFalse(enableBtn.exists)
    XCTAssertFalse(nextBtn.isEnabled)

    adjustSlider(untilSlider, to: 1.0)
    let untiltfm = app.textFields["nrvc_until_minutes"]
    assertValueWithinTolerance(untiltfm.value as! String, expected: 59, tolerance: 2, message: "Until minutes after slider set to 1.0")
    let untiltfh = app.textFields["nrvc_until_hrs"]
    assertValueWithinTolerance(untiltfh.value as! String, expected: 11, tolerance: 1, message: "Until hours after slider set to 1.0")

    let atSlider = app.sliders["nrvc_at_slider"]
    adjustSlider(atSlider, to: 0.0)
    sleep(1)
    assertValueWithinTolerance(attfh.value as! String, expected: 12, tolerance: 1, message: "At time hours after slider set to 0.0")
    ...
    assertValueWithinTolerance(attfh.value as! String, expected: 7, tolerance: 1, message: "At hours after next button tap")
    assertMinutesAtZero(attfm.value as! String, message: "At minutes after next button tap")
    assertValueWithinTolerance(untiltfh.value as! String, expected: 11, tolerance: 1, message: "Until hours after next button tap")
    assertMinutesAtZero(untiltfm.value as! String, message: "Until minutes after next button tap")
    ...
    nextBtn.tap()
    XCTAssert(wd3.isSelected)
    assertMinutesAtZero(untiltfm.value as! String, message: "Until minutes after second next button tap")
    assertValueWithinTolerance(untiltfh.value as! String, expected: 11, tolerance: 1, message: "Until hours after second next button tap")
    nextBtn.tap()
